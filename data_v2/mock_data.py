...
EVIDENCE: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "size": "2.3 GB (1,847 relevant events)", "tool": "KAPE + Chainsaw → Nuix (indexed)",
    ...
    },
    {
        ...
        "size": "512.3 GB", "tool": "FTK Imager 4.7 + EnCase 22.4 (write-blocked)",
        ...
        "custody_chain": [
            ...
            {"action": "ANALYZED",    "icon": "🔬", "by": "J. Nakamura",  "role": "Lead Examiner",        "ts": _ts(10.8, 0),  "note": "EnCase 22.4 artefact analysis: deleted file recovery, MFT parsing, prefetch/shimcache/amcache extracted. OSForensics user activity timeline built.", "verified": True},
        ],
    },
    {
        "id": "EV-2024-001-04", "case_id": "CASE-2024-001",
        "type": "Mobile Device Image", "icon": "📱",
        "description": "Full filesystem extraction — CFO iPhone 14 Pro (suspected data exfil device)",
        "size": "128 GB (physical extraction)", "tool": "Magnet AXIOM 7.x (GrayKey assisted)",
        "sha256": "f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8b9c0d1e2f3a4",
        "md5": "1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d",
        "acquired_by": "M. Rodriguez", "acquired_at": _ts(10.5, 0), "integrity": True,
        "custody_chain": [
            {"action": "ACQUIRED",    "icon": "🔒", "by": "M. Rodriguez", "role": "DFIR Analyst",         "ts": _ts(10.5, 0),  "note": "Magnet AXIOM 7.x full filesystem extraction via GrayKey. iOS 17.4. Device in Faraday bag prior to acquisition.", "verified": True},
            {"action": "HASHED",      "icon": "🔐", "by": "M. Rodriguez", "role": "DFIR Analyst",         "ts": _ts(10.48, 0), "note": "SHA-256 + MD5 computed on acquisition. AXIOM case file sealed.", "verified": True},
            {"action": "TRANSFERRED", "icon": "📦", "by": "A. Petrov",    "role": "Evidence Custodian",   "ts": _ts(10.4, 0),  "note": "Uploaded to EY secure evidence vault (AES-256). AXIOM case file + raw image.", "verified": True},
            {"action": "VERIFIED",    "icon": "✅", "by": "A. Petrov",    "role": "Evidence Custodian",   "ts": _ts(10.35, 0), "note": "Hash verified on upload — SHA-256 PASS. AXIOM case integrity confirmed.", "verified": True},
            {"action": "ANALYZED",    "icon": "🔬", "by": "M. Rodriguez", "role": "DFIR Analyst",         "ts": _ts(9.5, 0),   "note": "AXIOM Artifact Explorer: 847 messages, iCloud Drive sync logs, 12 sensitive document transfers to personal Dropbox confirmed. Location data corroborates on-site presence during breach window.", "verified": True},
        ],
    },
    {
        "id": "EV-2024-002-01", "case_id": "CASE-2024-002",
        "type": "Email Archive (Nuix Processing)", "icon": "📧",
        "description": "Nuix-processed email corpus — APT29 supply chain compromise investigation",
        "size": "847 GB (1.2M items after Nuix dedup/filter)", "tool": "Nuix Workstation 9.10",
        "sha256": "a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2",
        "md5": "2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e",
        "acquired_by": "S. Okonkwo", "acquired_at": _ts(8.0, 0), "integrity": True,
        "custody_chain": [
            {"action": "ACQUIRED",    "icon": "🔒", "by": "S. Okonkwo",   "role": "Lead Examiner",        "ts": _ts(8.0, 0),   "note": "Exchange Online export via eDiscovery portal. PST archives + journal. Collected under court order #2024-CR-0892.", "verified": True},
            {"action": "HASHED",      "icon": "🔐", "by": "S. Okonkwo",   "role": "Lead Examiner",        "ts": _ts(7.98, 0),  "note": "SHA-256 computed on raw PST files prior to Nuix ingestion. Hash log filed.", "verified": True},
            {"action": "ANALYZED",    "icon": "🔬", "by": "S. Okonkwo",   "role": "Lead Examiner",        "ts": _ts(7.5, 0),   "note": "Nuix Workstation 9.10: 1.2M items processed, deduped, indexed. 847 responsive items tagged. Concept clustering identified 3 supply chain communication threads. Keyword hits: 'update package', 'deployment script', 'VPN credentials'.", "verified": True},
            {"action": "TRANSFERRED", "icon": "📦", "by": "A. Petrov",    "role": "Evidence Custodian",   "ts": _ts(7.4, 0),   "note": "Nuix case file exported to EY review platform. Bates-numbered production set generated.", "verified": True},
            {"action": "ARCHIVED",    "icon": "🗄",  "by": "System",       "role": "Automated",            "ts": _ts(7.0, 0),   "note": "Archived to EY long-term evidence storage (7-year retention).", "verified": True},
        ],
    },
]
